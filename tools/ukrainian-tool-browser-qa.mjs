...
const artifactDir = path.join(root, 'artifacts', 'ukrainian-localization-review');
const tools = [
...
try {
  for (const tool of tools) {
    for (const width of widths) {
      ...
      const response = await page.goto(`http://127.0.0.1:${port}/uk/${tool}/`, { waitUntil: 'networkidle' });
      ...
      if (!response?.ok() || result.lang !== 'uk' || result.horizontalOverflow > 1 || !result.h1Visible || !result.badgeLoaded ||
  ...
  for (const width of widths) {
    await page.setViewportSize({ width, height: 1000 });
    await page.goto(`http://127.0.0.1:${port}/uk/`, { waitUntil: 'networkidle' });
    const navigation = await page.evaluate(() => {
      ...
      return {
        count: cards.length,
        localizedLinks: cards.every(card => new URL(card.href).pathname.startsWith('/uk/')),
    ...
    if (navigation.count !== 7 || !navigation.localizedLinks || navigation.lastLabel !== 'Калькулятор бюджету подорожі' ||
        navigation.lastArrowVisible || !navigation.intermediateArrowsVisible) {
      throw new Error(`Ukrainian tool navigation @ ${width}: ${JSON.stringify(navigation)}`);
    }
  }

  await page.setViewportSize({ width: 390, height: 1000 });
  await page.goto(`http://127.0.0.1:${port}/uk/currency-converter/`, { waitUntil: 'networkidle' });
  ...
  if (!await page.locator('.result-date').textContent().then(text => text.includes('Довідкові курси'))) throw new Error('Currency result date was not localized');
  interactionChecks.push('currency');

  await page.goto(`http://127.0.0.1:${port}/uk/exchange-rate-markup-calculator/`, { waitUntil: 'networkidle' });
  ...
  if (positive.source !== 'EUR — Євро' || positive.target !== 'RSD — Сербський динар' || positive.label !== 'Ви отримаєте менше на' || !positive.percentage?.includes('5')) {
  ...
  if (negative.label !== 'Ви отримаєте більше на' || !negative.percentage?.includes('5')) throw new Error(`Negative markup interaction: ${JSON.stringify(negative)}`);
  interactionChecks.push('markup +5%', 'markup -5% core / higher-offer UI branch');

  await page.goto(`http://127.0.0.1:${port}/uk/multi-currency-converter/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/uk/exchange-rate-history/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/uk/foreign-transaction-fee-calculator/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/uk/travel-budget-calculator/`, { waitUntil: 'networkidle' });
  ...
  await failurePage.goto(`http://127.0.0.1:${port}/uk/currency-converter/`, { waitUntil: 'domcontentloaded' });
  ...
  if (failureText !== 'Не вдалося завантажити курси. Перевірте інтернет-з’єднання та повторіть спробу.') throw new Error(`Network failure message: ${failureText}`);
...
console.log(`Ukrainian browser QA PASS: ${layoutChecks.length} responsive checks, ${interactionChecks.length} interaction checks, navigation, network failure.`);
