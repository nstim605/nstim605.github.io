...
const artifactDir = path.join(root, 'artifacts', 'bosnian-localization-review');
const tools = [
...
try {
  for (const tool of tools) {
    for (const width of widths) {
      ...
      const response = await page.goto(`http://127.0.0.1:${port}/bs/${tool}/`, { waitUntil: 'networkidle' });
      ...
      if (!response?.ok() || result.lang !== 'bs' || result.horizontalOverflow > 1 || !result.h1Visible || !result.badgeLoaded ||
  ...
  for (const width of widths) {
    await page.setViewportSize({ width, height: 1000 });
    await page.goto(`http://127.0.0.1:${port}/bs/`, { waitUntil: 'networkidle' });
    const navigation = await page.evaluate(() => {
      ...
      return {
        count: cards.length,
        localizedLinks: cards.every(card => new URL(card.href).pathname.startsWith('/bs/')),
    ...
    if (navigation.count !== 7 || !navigation.localizedLinks || navigation.lastLabel !== 'Kalkulator budžeta putovanja' ||
        navigation.lastArrowVisible || !navigation.intermediateArrowsVisible) {
      throw new Error(`Bosnian tool navigation @ ${width}: ${JSON.stringify(navigation)}`);
    }
  }

  await page.setViewportSize({ width: 390, height: 1000 });
  await page.goto(`http://127.0.0.1:${port}/bs/currency-converter/`, { waitUntil: 'networkidle' });
  ...
  if (!await page.locator('.result-date').textContent().then(text => text.includes('Referentni kursevi'))) throw new Error('Currency result date was not localized');
  interactionChecks.push('currency');

  await page.goto(`http://127.0.0.1:${port}/bs/exchange-rate-markup-calculator/`, { waitUntil: 'networkidle' });
  ...
  if (positive.source !== 'EUR — Euro' || positive.target !== 'RSD — Srpski dinar' || positive.label !== 'Manje primate' || !positive.percentage?.includes('5')) {
  ...
  if (negative.label !== 'Više primate' || !negative.percentage?.includes('5')) throw new Error(`Negative markup interaction: ${JSON.stringify(negative)}`);
  interactionChecks.push('markup +5%', 'markup -5% core / higher-offer UI branch');

  await page.goto(`http://127.0.0.1:${port}/bs/multi-currency-converter/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/bs/exchange-rate-history/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/bs/foreign-transaction-fee-calculator/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/bs/travel-budget-calculator/`, { waitUntil: 'networkidle' });
  ...
  await failurePage.goto(`http://127.0.0.1:${port}/bs/currency-converter/`, { waitUntil: 'domcontentloaded' });
  ...
  if (failureText !== 'Kurseve nije moguće učitati. Provjerite internetsku vezu i pokušajte ponovo.') throw new Error(`Network failure message: ${failureText}`);
...
console.log(`Bosnian browser QA PASS: ${layoutChecks.length} responsive checks, ${interactionChecks.length} interaction checks, navigation, network failure.`);
