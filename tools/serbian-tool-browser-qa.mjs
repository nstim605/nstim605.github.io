...
const artifactDir = path.join(root, 'artifacts', 'serbian-localization-review');
const tools = [
...
try {
  for (const tool of tools) {
    for (const width of widths) {
      ...
      const response = await page.goto(`http://127.0.0.1:${port}/sr/${tool}/`, { waitUntil: 'networkidle' });
      ...
      if (!response?.ok() || result.lang !== 'sr' || result.horizontalOverflow > 1 || !result.h1Visible || !result.badgeLoaded ||
  ...
  for (const width of widths) {
    await page.setViewportSize({ width, height: 1000 });
    await page.goto(`http://127.0.0.1:${port}/sr/`, { waitUntil: 'networkidle' });
    const navigation = await page.evaluate(() => {
      ...
      return {
        count: cards.length,
        localizedLinks: cards.every(card => new URL(card.href).pathname.startsWith('/sr/')),
    ...
    if (navigation.count !== 7 || !navigation.localizedLinks || navigation.lastLabel !== 'Калкулатор буџета путовања' ||
        navigation.lastArrowVisible || !navigation.intermediateArrowsVisible) {
      throw new Error(`Serbian tool navigation @ ${width}: ${JSON.stringify(navigation)}`);
    }
  }

  await page.setViewportSize({ width: 390, height: 1000 });
  await page.goto(`http://127.0.0.1:${port}/sr/currency-converter/`, { waitUntil: 'networkidle' });
  ...
  if (!await page.locator('.result-date').textContent().then(text => text.includes('Референтни курсеви'))) {
    throw new Error('Currency result date was not localized');
  }
  interactionChecks.push('currency');

  await page.goto(`http://127.0.0.1:${port}/sr/exchange-rate-markup-calculator/`, { waitUntil: 'networkidle' });
  ...
  const positiveMarkup = await page.evaluate(() => ({
  ...
  if (!/^EUR — [Ее]вро$/.test(positiveMarkup.sourceOption ?? '') || !/^RSD — [Сс]рпски динар$/.test(positiveMarkup.targetOption ?? '') ||
      positiveMarkup.label !== 'Добијате мање за' || !positiveMarkup.percentage?.includes('5')) {
    throw new Error(`Positive markup interaction: ${JSON.stringify(positiveMarkup)}`);
  }
  await page.locator('#offered-amount').fill('12 285');
  await page.locator('#markup-form button[type="submit"]').click();
  const negativeMarkup = await page.evaluate(() => ({
    label: document.querySelector('[data-label="difference"]')?.textContent,
    percentage: document.querySelector('[data-result="percentage"]')?.textContent
  }));
  // The core calculation is -5%; the UI intentionally renders its absolute
  // magnitude and communicates direction through the localized row label.
  if (negativeMarkup.label !== 'Добијате више за' || !negativeMarkup.percentage?.includes('5')) {
    throw new Error(`Negative markup interaction: ${JSON.stringify(negativeMarkup)}`);
  }
  interactionChecks.push('markup +5%', 'markup -5% core / higher-offer UI branch');

  await page.goto(`http://127.0.0.1:${port}/sr/multi-currency-converter/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/sr/exchange-rate-history/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/sr/foreign-transaction-fee-calculator/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/sr/travel-budget-calculator/`, { waitUntil: 'networkidle' });
  ...
  await failurePage.goto(`http://127.0.0.1:${port}/sr/currency-converter/`, { waitUntil: 'domcontentloaded' });
  ...
  if (failureText !== 'Курсеви нису могли да се учитају. Проверите интернет везу и покушајте поново.') {
...
console.log(`Serbian browser QA PASS: ${layoutChecks.length} responsive checks, ${interactionChecks.length} interaction checks, navigation, network failure.`);
