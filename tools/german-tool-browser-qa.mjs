...
const artifactDir = path.join(root, 'artifacts', 'german-localization-review');
const tools = [
...
try {
  for (const tool of tools) {
    for (const width of widths) {
      ...
      const response = await page.goto(`http://127.0.0.1:${port}/de/${tool}/`, { waitUntil: 'networkidle' });
      const result = await page.evaluate(() => {
        ...
        return {
          ...
          badgeLabel: badge?.closest('a')?.getAttribute('aria-label'),
  ...
  for (const width of widths) {
    await page.setViewportSize({ width, height: 1100 });
    await page.goto(`http://127.0.0.1:${port}/de/`, { waitUntil: 'networkidle' });
    const navigation = await page.evaluate(() => {
      ...
      return {
        count: cards.length,
        localizedLinks: cards.every(card => new URL(card.href).pathname.startsWith('/de/')),
        lastLabel: cards.at(-1)?.textContent.replace('→', '').trim(),
        lastArrowVisible: Boolean(lastArrow && getComputedStyle(lastArrow).display !== 'none'),
        intermediateArrowsVisible: cards.slice(0, -1).every(card => {
          const arrow = card.querySelector('[aria-hidden="true"]');
          return Boolean(arrow && getComputedStyle(arrow).display !== 'none');
        })
      };
    });
    if (navigation.count !== 7 || !navigation.localizedLinks || navigation.lastLabel !== 'Reisebudgetrechner' ||
        navigation.lastArrowVisible || !navigation.intermediateArrowsVisible) {
      throw new Error(`German tool navigation @ ${width}: ${JSON.stringify(navigation)}`);
    }
  }

  await page.setViewportSize({ width: 390, height: 1100 });
  await page.goto(`http://127.0.0.1:${port}/de/currency-converter/`, { waitUntil: 'networkidle' });
  const rejectConsent = page.locator('.analytics-consent-reject');
  if (await rejectConsent.isVisible()) await rejectConsent.click();
  await page.locator('#currency-amount').fill('1 000,5');
  await page.locator('#currency-form button[type="submit"]').click();
  await page.locator('#currency-results:not([hidden])').waitFor();
  const currencyResult = await page.locator('[data-result="amount"]').textContent();
  const currencyDate = await page.locator('.result-date').textContent();
  if (!currencyResult?.includes('117.058,5') || !currencyDate?.includes('8. Sept. 2026')) {
    throw new Error(`German currency formatting: ${currencyResult} / ${currencyDate}`);
  }
  formatChecks.push('1 000,5 → 117.058,5', '8. Sept. 2026');
  interactionChecks.push('currency');

  await page.goto(`http://127.0.0.1:${port}/de/exchange-rate-markup-calculator/`, { waitUntil: 'networkidle' });
  ...
  if (positive.source !== 'EUR — Euro' || positive.target !== 'RSD — Serbischer Dinar' || positive.label !== 'Sie erhalten weniger' || !positive.percentage?.includes('5')) {
    throw new Error(`Positive deviation interaction: ${JSON.stringify(positive)}`);
  }
  ...
  if (negative.label !== 'Sie erhalten mehr' || !negative.percentage?.includes('5')) throw new Error(`Negative deviation interaction: ${JSON.stringify(negative)}`);
  interactionChecks.push('deviation +5%', 'deviation -5% core / higher-offer UI branch');

  await page.goto(`http://127.0.0.1:${port}/de/multi-currency-converter/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/de/exchange-rate-history/`, { waitUntil: 'networkidle' });
  ...
  const historyText = await page.locator('#history-results').innerText();
  if (!historyText.includes('116,5')) throw new Error(`German history number formatting: ${historyText}`);
  formatChecks.push('116,5 historical rate');
  interactionChecks.push('history 30/90/365');

  await page.goto(`http://127.0.0.1:${port}/de/foreign-transaction-fee-calculator/`, { waitUntil: 'networkidle' });
  ...
  await page.goto(`http://127.0.0.1:${port}/de/travel-budget-calculator/`, { waitUntil: 'networkidle' });
  ...
  await failurePage.goto(`http://127.0.0.1:${port}/de/currency-converter/`, { waitUntil: 'domcontentloaded' });
  ...
  if (failureText !== 'Kurse konnten nicht geladen werden. Prüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.') {
...
const report = {
  ...
  numberAndDateFormatting: formatChecks,
  localizedInput: ['1 000,5', '11 115', '12 285', '2,5', '1 000', '11,5'],
  networkFailure: 'PASS', runtimeErrors: 0
};
await fs.writeFile(path.join(artifactDir, 'browser-qa.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log(`German browser QA PASS: ${layoutChecks.length} responsive checks, ${interactionChecks.length} interaction checks, navigation, number/date formatting, network failure.`);
