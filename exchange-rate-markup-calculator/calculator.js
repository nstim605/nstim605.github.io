...
function setBusy(busy) {
  ...
  submitButton.textContent = busy ? 'Loading latest rates…' : 'Calculate markup';
}
...
function renderResult(calculation, source, target, date) {
  ...
  results.querySelector('[data-label="difference"]').textContent = worse ? 'You receive less by' : 'You receive more by';
  results.querySelector('[data-label="percentage"]').textContent = worse ? 'Exchange-rate markup' : 'Above reference by';
  results.querySelector('.result-summary').textContent = worse
    ? `The offer is ${rows.percentage} below the reference result.`
    : `The offer is ${rows.percentage} above the reference result.`;
...
form.addEventListener('submit', async event => {
  ...
  if (!Number.isFinite(amount) || amount <= 0 || !Number.isFinite(offeredAmount) || offeredAmount <= 0) {
    setStatus('Enter two amounts greater than zero.', 'error');
    return;
  }
  ...
  try {
    ...
    if (!sourceRate || !targetRate) throw new Error('A reference rate is unavailable for this currency pair.');
    renderResult(calculateMarkup({ amount, offeredAmount, sourceRate, targetRate }), source, target, date);
    setStatus('Calculation updated from the latest available reference rates.', 'success');
